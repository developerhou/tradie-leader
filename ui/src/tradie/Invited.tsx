import React from "react";
import { Paper, Grid, Typography, Button, Avatar } from "@mui/material";
import { LocationOn, Category } from "@mui/icons-material";
import { Job } from "../entity";

interface InvitedJobCardProps {
  job: Job;
  onStatusUpdate: (jobId: number, newStatus: string) => void;
}

const InvitedJobCard: React.FC<InvitedJobCardProps> = ({
  job,
  onStatusUpdate,
}) => {
  const firstLetter = job.contactName.charAt(0).toUpperCase();

  return (
    <Paper className="job-card" style={{ width: "50%", padding: "16px" }}>
      <Grid container spacing={2}>
        <Grid item xs={8}>
          <Grid container alignItems="center">
            <Avatar sx={{ bgcolor: "orange" }}>{firstLetter}</Avatar>
            <Typography variant="subtitle2" style={{ marginLeft: "15px" }}>
              {job.contactName}
            </Typography>
            <Typography variant="caption" style={{ marginLeft: "15px" }}>
              {new Date(job.createdAt).toLocaleString()}
            </Typography>
          </Grid>
        </Grid>

        <Grid item xs={8}>
          <Grid container alignItems="center">
            <LocationOn fontSize="small" />
            <Typography variant="caption">{job.suburb.name}</Typography>
            <Category fontSize="small" style={{ marginLeft: "15px" }} />
            <Typography variant="caption">{job.category.name}</Typography>
            <Typography variant="body2" style={{ marginLeft: "15px" }}>
              ID: {job.id}
            </Typography>
          </Grid>
        </Grid>

        <Grid item xs={8}>
          <Typography variant="body2">{job.description}</Typography>
        </Grid>

        <Grid item xs={8}>
          <Grid container alignItems="center">
            <Button
              variant="contained"
              sx={{ backgroundColor: "orange" }}
              onClick={() => onStatusUpdate(job.id, "accepted")}
            >
              Accept
            </Button>
            <Button
              variant="contained"
              sx={{ backgroundColor: "grey", marginLeft: "15px" }}
              onClick={() => onStatusUpdate(job.id, "declined")}
            >
              Decline
            </Button>
            <Typography variant="h6" style={{ marginLeft: "15px" }}>
              ${job.price} Lead Invitation
            </Typography>
          </Grid>
        </Grid>
      </Grid>
    </Paper>
  );
};

export default InvitedJobCard;
